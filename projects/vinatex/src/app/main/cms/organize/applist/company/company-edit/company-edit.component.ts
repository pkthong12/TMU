...
import { BaseEditComponent, ICoreDropdownOption, ICorePageEditColumnComposition, ICorePageEditCRUD, ICoreFormSection, EnumFormBaseContolType, EnumCoreFormControlSeekerSourceType, AppService, DialogService, MultiLanguageService, IFormatedResponse, AlertService, alertOptions, ICoreButtonVNS } from 'ngx-histaff-alpha';
...
export class CompanyEditComponent extends BaseEditComponent implements OnInit, OnDestroy {
  ...
  sections: ICoreFormSection[] = [
    {
      rows: [
        ...
        [
          ...
          {
            flexSize: 4,
            label: EnumTranslateKey.UI_COMPONENT_LABEL_COMPANY_STOCK_CODE,
            field: 'stockCode',
            value: '',
            controlType: EnumFormBaseContolType.TEXTBOX,
            readonly: false,
  ...
  onFormCreated(e: FormGroup): void {
    this.form = e;
    this.form.get('stockCode')?.valueChanges.pipe(distinctUntilChanged()).subscribe(x => {
      if (!!x) {
       this.checkSpecialCharacters(x);
  ...
  checkSpecialCharacters(e : any){
    const specialSring: RegExp = /[!@#$%^&*()_+{}|:"<>?[\];',./]/;
    if(specialSring.test(e)) {
      this.alertService.warn(this.mls.trans(EnumTranslateKey.UI_COMPONENT_TITLE_HU_COMPANY_ALERT_SPECIAL_CHARACTER), alertOptions)
    }
  }
