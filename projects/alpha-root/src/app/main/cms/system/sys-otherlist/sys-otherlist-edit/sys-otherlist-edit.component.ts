...
export class SysOtherlistEditComponent extends BaseEditComponent {
  ...
  protected static checkEffectDate(date: AbstractControl): any | null {
    let valid = true;
    let errorMessage = '';
    const startDatePlan = date.value;
    const endDatePlan = date.parent?.get('expirationDate')?.value;
    if (endDatePlan != '' && endDatePlan != null && startDatePlan != null) {
      if (startDatePlan > new Date(endDatePlan)) {
        ...
        return CustomValidators.core('effectiveDate', false, errorMessage)(date);
      } else {
        date.parent?.get('effectiveDate')?.setErrors(null);
        date.parent?.get('expirationDate')?.setErrors(null);
      }
    }
  }

  protected static checkExpirationDate(date: AbstractControl): any | null {
    let valid = true;
    let errorMessage = '';
    const startDatePlan = date.parent?.get('effectiveDate')?.value;
    ...
    if (endDatePlan != '' && endDatePlan != null) {
      if (startDatePlan != '' && startDatePlan != null && endDatePlan < new Date(startDatePlan)) {
        valid = false;
        errorMessage = EnumTranslateKey.UI_FORM_CONTROL_WELFARE_ERROR_TO_EXPIRATION_DATE_MORE_THAN_EFFECT_DATE;
        return CustomValidators.core('expirationDate', false, errorMessage)(date);
      } else {
        date.parent?.get('effectiveDate')?.setErrors(null);
        date.parent?.get('expirationDate')?.setErrors(null);
      }
  ...
  //UI_FORM_CONTROL_WELFARE_ERROR_TO_EXPIRATION_DATE_MORE_THAN_EFFECT_DATE
