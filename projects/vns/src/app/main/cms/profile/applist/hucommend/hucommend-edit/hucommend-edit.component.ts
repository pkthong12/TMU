...
export class HucommendEditComponent extends BaseEditComponent implements OnDestroy, AfterViewInit {
  ...
  onFormSubmit() {
    ...
    if (!!this.huCommendEditService.commendId) {
      if (!!this.form.valid) {
        this.huCommendEditService.update(request).subscribe((x: any) => {
      ...
      if (!!this.form.valid) {
        this.huCommendEditService.create(request).subscribe((rs: any) => {
