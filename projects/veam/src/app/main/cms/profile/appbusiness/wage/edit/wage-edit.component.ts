...
export class WageEditComponent extends BaseEditComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  salLevelDCVGetByIdApi = api.HU_SALARY_LEVEL_READ;
  ...
  ngAfterViewInit(): void {
    ...
    this.form.get("employeeId")?.valueChanges.subscribe(x => {
      ...
      if (!!effectDate && effectDate != null) {
        ...
        this.appService.post(api.GET_SALARY_MINIMUM_OF_REGION, payload).subscribe(res => {
          var data = res.body.innerBody.toLocaleString('en-US', { style: 'decimal' });

          this.form.get("regionMinimumWage")?.patchValue(data);
        });
      }
    });
    ...
    this.form.get("coefficient")?.valueChanges.subscribe(x => {
      if (!!x) {
        ...
        if (!!regionMinimumWage) {
          var regionMinimumWage2;
          
          switch (typeof regionMinimumWage) {
            case "number":
              regionMinimumWage2 = regionMinimumWage;
              break;

            case "string":
              regionMinimumWage2 = parseInt(regionMinimumWage.replace(/,/g, ''));
              break;
          ...
          var salInsu = Math.floor(regionMinimumWage2 * x);

          var data = salInsu.toLocaleString('en-US', { style: 'decimal' });

          this.form.get("salInsu")?.patchValue(data);
        }
      }
      else {
    ...
    this.form.get("signId")?.valueChanges.subscribe(x => {
      if (x == null) {
        this.form.get("signerPosition")?.patchValue(null);
      }
    });
  }

  ngAfterViewChecked() {
    var salInsu = this.form.get("salInsu")?.value;

    if (!!salInsu && typeof salInsu == "number") {
      var data = salInsu.toLocaleString('en-US', { style: 'decimal' });
      ...
      this.form.get('salaryLevelDcvId')?.valueChanges.pipe(distinctUntilChanged())
        .subscribe(x => {
          if (!!x) {
            ...
            this.form.get('coefficientDcv')?.setValue(null);
          }
        })!
    ) // Close outer push  
