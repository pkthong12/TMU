            ...
            [
              {
                ...
                alsoBindTo: [
                  ...
                  { takeFrom: 'regionMinimumWage', bindTo: 'regionMinimumWage' },
                  { takeFrom: 'salInsu', bindTo: 'salInsu' },
                ],
                /* END: WHEN USING controlType === EnumFormBaseContolType.SEEKER */
                type: 'text',
                readonly: true,
    ...
    this.subsctiptions.push(
      this.form.get('startDate')?.valueChanges.pipe(
        ...
        if (!!x) {
          ...
          // cmt code vì
          // không tự động lấy hồ sơ lương nữa
            ...
            this.appService.get(api.HU_CONTRACT_GETWAYGEBYSTARTDATECONTRACT + this.form.get('employeeId')?.value + "&date=" + new Date(x).toLocaleDateString("en-US")).subscribe(x => {
              if (x.ok && x.status === 200) {
                ...
                if (body.statusCode === 200) {
                  ...
                  if (body.innerBody.regionMinimumWage !== undefined) {
                    this.form.get('regionMinimumWage')?.patchValue(body.innerBody.regionMinimumWage);
                  }
                  if (body.innerBody.salInsu !== undefined) {
                    this.form.get('salInsu')?.patchValue(body.innerBody.salInsu);
                  }
                } else {
