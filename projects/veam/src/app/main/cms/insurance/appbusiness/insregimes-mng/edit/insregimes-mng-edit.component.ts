  ...
  implements AfterViewInit, OnInit
{
  ...
  ngAfterViewInit(): void {
    ...
    this.form.get('insGroupId')?.valueChanges.subscribe(id => {
      if (!!this.regimesGroupOptions$) {
        this.regimesGroupOptions$.subscribe(data => {
          if (!!data && data.length > 0) {
            var opt;
  
            var arr = data.filter(item => item.value === id);
  
            if (!!arr && arr.length > 0) opt = arr[0];
  
            if (!!opt) {
              if (opt.code === 'NCD001') this.form.get('childrenNo')?.enable();
